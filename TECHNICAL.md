...
- No AI evaluation dataset/metrics beyond the fixture-based unit tests — worth having given real extraction has occasionally shown minor date-inference imprecision (e.g. "this Friday" resolving to a date that doesn't obviously match) in ad hoc live testing; nothing formally measured
...
- Send-failure **reconciliation is unverified under an actually-forced failure** — the happy path (send succeeds, exactly one email lands) is confirmed live; deliberately injecting a mid-send failure to watch `NEEDS_RECONCILIATION` → retry → dedupe actually catch it has not been done
